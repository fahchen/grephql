defmodule TypedGql.Macros do
  ...
  @doc false
  @spec __build_doc__(TypedGql.Query.t()) :: String.t()
  def __build_doc__(%TypedGql.Query{} = query) do
  ...
  # so we pattern-match the AST node and extract the binary string. Its meta is
  # what puts the document's own lines in this file.
  ...
  # interpolated value may itself contain newlines, so no document line maps
  # back to a line of this file: the modules keep the caller's own location.
  defp define_query_function(kind, func_name, {:<<>>, _meta, _parts} = query_str_ast)
       when is_atom(func_name) do
    build_query_ast(kind, func_name, query_str_ast, nil)
  end

  # A plain string is anchorless for the same reason: a lowercase sigil or a
  # heredoc written without `~GQL` arrives here already interpolated.
  ...
  defp build_query_ast(kind, func_name, query_str_ast, base) do
    ...
    quote bind_quoted: [func_name: func_name, query_str: query_str_ast, base: Macro.escape(base)],
      ...
      @typed_gql_query TypedGql.Compiler.compile!(
                         typed_gql_full_query,
                         @typed_gql_schema,
                         client_module: __MODULE__,
  ...
  defp build_function_ast(kind, func_name) do
    doc_ast =
      if kind == :def, do: quote(do: @doc(TypedGql.Macros.__build_doc__(@typed_gql_query)))

    quote do
      unquote(doc_ast)

      if @typed_gql_query.has_variables? do
        TypedGql.Macros.__define_spec_with_vars__(
          unquote(func_name),
          @typed_gql_query.result_module,
          @typed_gql_query.variables_module
        )

        unquote(func_with_variables_ast(kind, func_name))
      else
        TypedGql.Macros.__define_spec_without_vars__(
          unquote(func_name),
          @typed_gql_query.result_module
        )
  ...
  defp func_with_variables_ast(:def, name) do
    quote do
      def unquote(name)(variables, opts \\ []) do
        TypedGql.Macros.__execute_with_variables__(@typed_gql_query, variables, opts)
      end
    end
  end
  ...
  defp func_without_variables_ast(:def, name) do
    quote do
      def unquote(name)(opts \\ []) do
        TypedGql.execute(@typed_gql_query, %{}, opts)
      end
    end
  end
