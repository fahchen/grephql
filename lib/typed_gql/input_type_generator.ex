defmodule TypedGql.InputTypeGenerator do
  ...
  it, and a nested input object records the variable that pulled it in. A type
  two `defgql`s share is generated once, by the first of them, and keeps that
  one's location: the module already exists when the second runs.
  """
  ...
  def generate(operation, schema, opts) do
    ...
    # validates an embed's module at schema-compile time. `generate_variables/3`
    # then rebuilds each of them anchored to the variable definition that pulled
    # it in, and that copy is the one that survives — so there is no location to
    # thread here, only the definition.
    {modules, module_asts, _seen} =
      ...
      |> Enum.reduce({[], [], MapSet.new()}, fn type_name, collect_acc ->
        collect_input_type(
          type_name,
          {schema, scalar_types, inputs_module, caller_env},
          collect_acc
        )
      end)
  ...
  # Paired with the variable definition that referenced the type, since the type
  # itself comes from the schema and has no location. Two variables of the same
  # type generate one module, located at the first of them.
