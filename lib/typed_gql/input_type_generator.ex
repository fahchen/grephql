defmodule TypedGql.InputTypeGenerator do
  ...
  A type two `defgql`s share is generated by both — the `Code.ensure_loaded?/1`
  guard does not skip the second, because the parallel compiler has not made
  the first's module loadable yet — so the location that survives is the last
  one's, not the first's.
  """
  ...
  # Names only: an input type comes from the schema and has no location of its
  # own, and the copy of it that survives is the one `generate_variables/3`
  # rebuilds, anchored to the variable definition that pulled it in.
